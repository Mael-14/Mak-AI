...
/**
 * MathText Component - Renders text with LaTeX/MathJax support
 * Detects LaTeX patterns and renders them properly
 */
const MathText = ({ text, style, isError }) => {
  ...
  useEffect(() => {
    ...
    const containsLatex = latexPatterns.some(pattern => pattern.test(text));
    setHasLatex(containsLatex);

    if (containsLatex) {
      // Convert text to HTML with MathJax
      const html = generateMathJaxHTML(text, isError);
      setHtmlContent(html);
    }
  }, [text, isError]);

  // If no LaTeX, render as regular text
  if (!hasLatex) {
    return (
      <Text style={[styles.formattedText, style, isError && styles.errorText]}>
        {text}
      </Text>
    );
  }

  // Render with MathJax WebView
  return (
      ...
      <WebView
        ...
        onMessage={(event) => {
          try {
            ...
            if (data.height) {
              setWebViewHeight(data.height + 20); // Add padding
            }
          } catch (e) {
 ...
 * Supports: headers, bold, italic, lists, code blocks, blockquotes, links, etc.
 */
const parseMarkdown = (text) => {
  ...
  let html = text
    ...
    .map((part, i, arr) => {
      ...
      return part; // Code block - don't escape
    })
    .join('');
  ...
  const AiMessage = ({ title, text, time, isError }) => (
    <View style={styles.aiMessageContainer}>
      <View style={[styles.aiContent, isError && styles.errorContent]}>
        ...
        <MathText text={text} isError={isError} />
