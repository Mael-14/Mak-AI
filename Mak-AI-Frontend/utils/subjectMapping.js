/**
 * Subject ID to Subject Code Mapping
 * Maps the frontend subject IDs (1-8) to GCE subject codes
 * 
 * Subject Codes Reference:
 * - Ordinary Level (O-Level): Typically 4-digit codes (e.g., 0570, 0610)
 * - Advance Level (A-Level): Typically 4-digit codes (e.g., 9700, 9701)
 */

export const SUBJECT_ID_TO_CODE = {
  ...
  6: {
    name: 'Math Stats',
    ordinaryLevel: '4040', // Statistics (O-Level) - Note: May need adjustment
    advanceLevel: '9709',  // Statistics (A-Level) - Often part of Mathematics
  },
  7: {
    name: 'Geography',
 ...
 */
export const getSubjectCode = (subjectId, level) => {
...
/**
 * Get all subject codes for a given level
 * @param {string} level - The level ('Ordinary Level' or 'Advance Level')
 * @returns {Object} Object mapping subject IDs to subject codes
 */
export const getAllSubjectCodesForLevel = (level) => {
  const codes = {};
  Object.keys(SUBJECT_ID_TO_CODE).forEach((id) => {
    const code = getSubjectCode(parseInt(id), level);
    if (code) {
      codes[id] = code;
    }
  });
...
export default SUBJECT_ID_TO_CODE;
